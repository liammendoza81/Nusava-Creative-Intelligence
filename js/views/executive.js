...
/* ── Subscriptions Panel — TikTok recurring revenue.
   Pulls window.SUBSCRIPTIONS (generated by pipeline.parsers.subscription_dump).
   Highlights the strategic insight: ~50% of TikTok GMV is now subscription/
   recurring revenue with ~90% week-over-week retention.
   ─────────────────────────────────────────────────── */
function buildSubscriptionsPanel() {
  var S = window.SUBSCRIPTIONS;
  if (!S || !Array.isArray(S.brand_weekly) || !S.brand_weekly.length) return '';
  var brand = S.brand_weekly;
  var latest = brand[brand.length - 1];
  var prev = brand.length > 1 ? brand[brand.length - 2] : null;
  var fourBack = brand.length > 4 ? brand[brand.length - 5] : null;
  ...
  var deltaPct = function (curr, prev, kind) {
  ...
  // 8-week change for active subscribers (trend signal)
  var growthVs4w = (fourBack && fourBack.active_subscribers) ?
    ((latest.active_subscribers - fourBack.active_subscribers) / fourBack.active_subscribers * 100) : null;

  var kpis =
    kpiCard('Active Subscriptions',
      fmt(latest.active_subscriptions, 'int'),
      deltaPct(latest.active_subscriptions, prev ? prev.active_subscriptions : 0) +
      (growthVs4w != null ? ' · ' + (growthVs4w >= 0 ? '+' : '') + growthVs4w.toFixed(1) + '% vs 4w ago' : ''),
      'green') +
    kpiCard('Subscription GMV Share',
      fmt(latest.subscription_gmv_share, 'pct'),
      'of TikTok video GMV is recurring · $' + Math.round(latest.subscription_gmv).toLocaleString() + ' this week',
      latest.subscription_gmv_share >= 0.4 ? 'green' : latest.subscription_gmv_share >= 0.25 ? 'yellow' : 'orange') +
    kpiCard('Retention Rate',
      fmt(latest.retention_rate, 'pct'),
      'WoW subscriber retention',
      latest.retention_rate >= 0.90 ? 'green' : latest.retention_rate >= 0.85 ? 'yellow' : 'red') +
    kpiCard('Take Rate',
      fmt(latest.take_rate, 'pct'),
      'New subscriptions ÷ first-time customers',
      latest.take_rate >= 0.20 ? 'green' : latest.take_rate >= 0.12 ? 'yellow' : 'orange') +
    kpiCard('New Subscriptions',
      fmt(latest.new_subscriptions, 'int'),
      'this week · ' + deltaPct(latest.new_subscriptions, prev ? prev.new_subscriptions : 0),
      'green') +
    kpiCard('Recurring Share of Subs',
      fmt(latest.recurring_share, 'pct'),
      '(active − new) ÷ active · % from prior subscribers',
      latest.recurring_share >= 0.85 ? 'green' : 'yellow');

  return '<div class="section-header" style="margin-top:32px">' +
    '<span class="section-title">Subscriptions — Recurring Revenue</span>' +
    '<span class="section-meta">Week ending ' + latest.week_end + ' · ' + brand.length + ' weeks tracked · ~' + Math.round(latest.subscription_gmv_share * 100) + '% of TikTok GMV is recurring</span>' +
    '</div>' +
    '<div class="kpi-grid">' + kpis + '</div>' +
    '<div class="chart-card" style="margin-top:14px">' +
      '<div class="chart-wrap" style="height:240px"><canvas id="exec-subs-chart"></canvas></div>' +
    '</div>';
}
...
function drawSubsChart() {
  var S = window.SUBSCRIPTIONS;
  if (!S || !Array.isArray(S.brand_weekly) || !S.brand_weekly.length) return;
  var canvas = document.getElementById('exec-subs-chart');
  if (!canvas || typeof Chart === 'undefined') return;

  var brand = S.brand_weekly;
  var labels = brand.map(function (b) { return b.week_end.slice(5); });   // MM-DD
  var subs = brand.map(function (b) { return b.active_subscriptions; });
  var share = brand.map(function (b) { return b.subscription_gmv_share * 100; });
  var retention = brand.map(function (b) { return b.retention_rate * 100; });
    ...
    options: {
      ...
      scales: {
        y: {
          beginAtZero: false,
          position: 'left',
          title: { display: true, text: 'Active Subscriptions', font: { size: 11 } },
        ...
        y1: {
          ...
          title: { display: true, text: '% (Share / Retention)', font: { size: 11 } },
